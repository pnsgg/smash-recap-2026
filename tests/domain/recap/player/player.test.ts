...
describe('Player', () => {
  ...
  describe("tournamentsByMonth", () => {
    ...
    test("should contains values if the player did attend tournaments this year", () => {
      const player = PlayerFactory.merge({
        tournaments: [
          ...TournamentFactory.merge({ startDate: new Date("2026-01-01") }).makeMany(100),
          ...TournamentFactory.merge({ startDate: new Date("2026-02-01") }).makeMany(50),
          ...TournamentFactory.merge({ startDate: new Date("2026-03-01") }).makeMany(25),
          ...TournamentFactory.merge({ startDate: new Date("2026-04-01") }).makeMany(17),
          ...TournamentFactory.merge({ startDate: new Date("2026-05-01") }).makeMany(71),
          ...TournamentFactory.merge({ startDate: new Date("2026-06-01") }).makeMany(35),
          ...TournamentFactory.merge({ startDate: new Date("2026-07-01") }).makeMany(70),
          ...TournamentFactory.merge({ startDate: new Date("2026-08-01") }).makeMany(0),
          ...TournamentFactory.merge({ startDate: new Date("2026-09-01") }).makeMany(1),
          ...TournamentFactory.merge({ startDate: new Date("2026-10-01") }).makeMany(2),
          ...TournamentFactory.merge({ startDate: new Date("2026-11-01") }).makeMany(4),
          ...TournamentFactory.merge({ startDate: new Date("2026-12-01") }).makeMany(8),
